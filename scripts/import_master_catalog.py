...
EXERCISES += [
    ...
    E("Сгибание ног на мяче", "Лёжа на спине, пятки на фитболе, подъём таза и подкат мяча сгибанием голени.", stats=("strength",), patterns=["hip_hinge"], muscles=HINGE_M, equip=["fitball"], diff=2, vtype="sr", vol=(3, 10, 12)),
...
EXERCISES += [
    ...
    E("Толкание саней", "Сани толкаются вперёд руками из наклонного положения корпуса на дистанцию.", stats=("strength", "agility"), patterns=["squat"], muscles=FULLBODY_POWER_M, equip=["sled"], stimulus="power", diff=2, vtype="sr", vol=(4, 1, 1)),
    E("Тяга саней", "Сани тянутся на ремне/верёвке шагом назад или вперёд на дистанцию.", stats=("strength", "agility"), patterns=["hip_hinge"], muscles=FULLBODY_POWER_M, equip=["sled"], stimulus="power", diff=2, vtype="sr", vol=(4, 1, 1)),
]
...
EXERCISES += [
    ...
    E("Тяга саней спиной вперёд", "Движение спиной вперёд с сопротивлением саней, акцент на квадрицепс и контроль торможения.", stats=("strength", "agility"), patterns=["squat"], muscles=SQUAT_M, equip=["sled"], stimulus="power", diff=3, vtype="sr", vol=(4, 1, 1)),
...
EXERCISES += [
    ...
    E("Мешай в котле", "Упор предплечьями на фитбол в планке, круговые движения предплечьями, сохраняя стабильный таз.", stats=("strength",), patterns=["core"], muscles=CORE_M, equip=["fitball"], diff=3, vtype="sr", vol=(3, 6, 6)),
    ...
    E("Планка на фитболе с перекатом", "Упор предплечьями на фитбол, прокат мяча вперёд-назад с сохранением прямой линии тела.", stats=("strength",), patterns=["core"], muscles=CORE_M, equip=["fitball"], diff=3, vtype="sr", vol=(3, 8, 10)),
...
# =========================================================================
# PART W -- puck handling / stick-handling drills (off-ice, phase=puck --
# ScheduleService._pick_puck_module_exercises' own tail-on pool, gated on
# owning a hockey stick). Primary stat is puck_handling everywhere unless
# noted otherwise; equipment is hockey_stick only, matching the 6
# pre-existing phase=puck exercises (Part N/Q above) -- no separate
# ball/puck-for-dribbling item exists, same precedent. Skill tags
# (Обводка/Баланс и устойчивость/Силовая борьба/Игровое чтение/Сила
# броска, with transfer notes) are NOT handled by this script's E()/runner
# -- see scripts/seed_puck_handling_skill_tags.py, run after this import.
# 2026-09-21.
# =========================================================================
EXERCISES += [
    E("Ведение мяча вокруг тела", "Стоя на месте, вести мяч клюшкой по кругу вокруг корпуса — спереди, сбоку, насколько позволяет длина клюшки, без остановки движения.", stats=("puck_handling", "agility"), patterns=["stick_handling", "coordination"], muscles={"forearms": 0.6, "core": 0.2, "shoulders": 0.2}, equip=["hockey_stick"], stimulus="skill", phase="puck", diff=1, vtype="dur", vol=30),
    E("Ведение восьмёркой между двух точек", "Поставить два предмета на расстоянии длины клюшки. Вести мяч передней стороной крюка вокруг первой точки к центру, затем задней стороной крюка вокруг второй, рисуя клюшкой траекторию восьмёрки без остановки. Смотреть вперёд, а не на мяч.", stats=("puck_handling", "agility"), patterns=["stick_handling", "coordination"], muscles={"forearms": 0.6, "core": 0.2, "shoulders": 0.2}, equip=["hockey_stick"], stimulus="skill", phase="puck", diff=1, vtype="dur", vol=30),
    E("Ведение по кругу с обходом точки", "Один предмет-ориентир на полу. Вести мяч по кругу вокруг него, полностью обходя точку клюшкой без потери контакта с мячом, затем обойти в обратную сторону.", stats=("puck_handling", "agility"), patterns=["stick_handling", "locomotion"], muscles={"forearms": 0.5, "core": 0.2, "calves": 0.2, "shoulders": 0.1}, equip=["hockey_stick"], stimulus="skill", phase="puck", uni=True, diff=1, vtype="sr", vol=(3, 6, 8)),
    E("Плавный перекат мяча из стороны в сторону", "Стоя на месте, широким плавным движением перекатывать мяч клюшкой слева направо и обратно, не отрывая крюк от мяча на всей амплитуде движения, регулируя силу нажима крюка так, чтобы мяч не укатывался дальше контролируемого расстояния.", stats=("puck_handling",), patterns=["stick_handling"], muscles={"forearms": 0.7, "shoulders": 0.2, "core": 0.1}, equip=["hockey_stick"], stimulus="skill", phase="puck", diff=1, vtype="dur", vol=30),
    E("Узкое ведение между двух ограничителей", "Два предмета лежат на расстоянии около 60 см друг от друга. Вести мяч максимально быстро в узком коридоре между ними, работая в основном верхней рукой, не задевая ограничители.", stats=("puck_handling", "agility"), patterns=["stick_handling", "coordination"], muscles={"forearms": 0.7, "shoulders": 0.2, "core": 0.1}, equip=["hockey_stick"], stimulus="skill", phase="puck", diff=2, vtype="dur", vol=20),
    E("Широкое ведение с максимальным разведением рук", "Вести мяч широкими быстрыми движениями от одного бедра к другому, максимально далеко разводя руки в стороны на каждом касании, сохраняя контроль.", stats=("puck_handling", "agility"), patterns=["stick_handling", "shoulder_mobility"], muscles={"shoulders": 0.5, "forearms": 0.3, "core": 0.2}, equip=["hockey_stick"], stimulus="skill", phase="puck", diff=2, vtype="dur", vol=30),
    E("Ведение вперёд-назад с приближением к телу", "Вести мяч, чередуя движение далеко вперёд от корпуса и близко к нему, меняя дистанцию до мяча на каждом такте, при этом сохраняя ширину движения в стороны.", stats=("puck_handling", "agility"), patterns=["stick_handling", "coordination"], muscles={"forearms": 0.6, "core": 0.2, "shoulders": 0.2}, equip=["hockey_stick"], stimulus="skill", phase="puck", diff=2, vtype="dur", vol=30),
    E("Ведение восьмёркой вблизи и вдали от тела", "Поставить два предмета на расстоянии длины клюшки, один ближе к ногам, другой на границе вытянутых рук впереди себя. Вести мяч восьмёркой между ними не вбок, а по глубине — то подтягивая мяч вплотную к ногам, то выкатывая его далеко вперёд на вытянутых руках, сохраняя непрерывное движение крюком.", stats=("puck_handling", "agility"), patterns=["stick_handling", "coordination"], muscles={"forearms": 0.6, "shoulders": 0.2, "core": 0.2}, equip=["hockey_stick"], stimulus="skill", phase="puck", diff=2, vtype="dur", vol=30),
    E("Ведение по четырём точкам (короб)", "Четыре предмета расставлены прямоугольником. Вести мяч, последовательно обходя каждую точку по периметру короба, сохраняя контроль на поворотах.", stats=("puck_handling", "agility"), patterns=["stick_handling", "locomotion"], muscles={"forearms": 0.5, "calves": 0.2, "core": 0.2, "shoulders": 0.1}, equip=["hockey_stick"], stimulus="skill", phase="puck", diff=2, vtype="sr", vol=(3, 2, 3)),
    E("Финт носком крюка на месте", "Мяч перед игроком. Крайним носком крюка резко подтянуть мяч к себе, затем быстро увести обратно наружу тем же носком, не переворачивая крюк.", stats=("puck_handling", "agility"), patterns=["stick_handling", "coordination"], muscles={"forearms": 0.7, "shoulders": 0.2, "core": 0.1}, equip=["hockey_stick"], stimulus="skill", phase="puck", uni=True, diff=3, vtype="sr", vol=(3, 10, 12)),
    E("Финт носком крюка в разных точках вокруг тела", "Мяч перед игроком. Крайним носком крюка резко подтянуть мяч к себе и увести обратно наружу тем же носком (финт), затем вернуть мяч в исходную точку и повторить то же движение ещё в четырёх разных положениях вокруг тела — спереди, с обоих боков и по двум диагоналям, — последовательно проходя все пять точек по кругу.", stats=("puck_handling", "agility"), patterns=["stick_handling", "coordination"], muscles={"forearms": 0.7, "shoulders": 0.2, "core": 0.1}, equip=["hockey_stick"], stimulus="skill", phase="puck", diff=3, vtype="sr", vol=(1, 2, 3)),
    E("Быстрые короткие касания мяча на месте", "Серия очень коротких, быстрых касаний мяча клюшкой на месте (2-3 касания подряд перед сменой направления). Выполнять сериями на максимальной частоте.", stats=("puck_handling", "on_ice_skating"), patterns=["stick_handling", "coordination"], muscles={"forearms": 0.7, "shoulders": 0.2, "core": 0.1}, equip=["hockey_stick"], stimulus="skill", phase="puck", diff=3, vtype="dur", vol=20),
    E("Боковое ведение с широким выпадом в сторону", "Вести мяч, одновременно выполняя широкий шаг-выпад в сторону ведения мяча, растягивая тело максимально далеко, затем возвращаясь в исходное и повторяя в другую сторону.", stats=("puck_handling", "agility"), patterns=["stick_handling", "squat"], muscles={"forearms": 0.4, "quads": 0.3, "glutes": 0.2, "core": 0.1}, equip=["hockey_stick"], stimulus="skill", phase="puck", uni=True, diff=3, vtype="sr", vol=(3, 8, 10)),
    E("Ведение утяжелённым мячом", "Взять более тяжёлый, чем стандартный, мяч и выполнять им произвольное ведение — восьмёрку, узкое и широкое ведение из стороны в сторону, — сохраняя ту же технику крюка, что и с обычным снарядом. Повышенный вес естественно заставляет работать медленнее и с большим усилием кисти.", stats=("strength", "puck_handling"), patterns=["stick_handling"], muscles={"forearms": 0.8, "shoulders": 0.2}, equip=["hockey_stick"], stimulus="strength", phase="puck", diff=3, vtype="dur", vol=30),
    E("Ведение облегчённым мячом на скорость", "Взять лёгкий мяч (например, деревянный шар) и выполнять им произвольное ведение — восьмёрку, узкое и широкое ведение из стороны в сторону, — на максимально доступной скорости рук, которую позволяет облегчённый вес снаряда.", stats=("puck_handling", "agility"), patterns=["stick_handling", "coordination"], muscles={"forearms": 0.7, "shoulders": 0.2, "core": 0.1}, equip=["hockey_stick"], stimulus="skill", phase="puck", diff=3, vtype="dur", vol=20),
    E("Ведение с обманным движением плечом", "Во время базового ведения добавить резкий обманный наклон плеча и корпуса в одну сторону, продолжая при этом вести мяч в исходном направлении или неожиданно сменить направление вслед за корпусом.", stats=("puck_handling", "agility"), patterns=["stick_handling", "coordination"], muscles={"forearms": 0.5, "core": 0.3, "shoulders": 0.2}, equip=["hockey_stick"], stimulus="skill", phase="puck", uni=True, diff=3, vtype="sr", vol=(3, 8, 10)),
    E("Ведение одной рукой (сильная сторона)", "Держать клюшку только верхней рукой, вести мяч в стороны и вперёд-назад, сохраняя контроль без помощи нижней руки. Выполняется ведущей (верхней) рукой при обычном хвате.", stats=("puck_handling", "strength"), patterns=["stick_handling", "coordination"], muscles={"forearms": 0.8, "shoulders": 0.2}, equip=["hockey_stick"], stimulus="skill", phase="puck", uni=True, diff=4, vtype="dur", vol=20),
    E("Ведение одной рукой (слабая сторона)", "Держать клюшку только той рукой, которая обычно находится снизу при двуручном хвате (недоминантная рука), вести мяч в стороны и вперёд-назад, сохраняя контроль без помощи второй руки.", stats=("puck_handling", "strength"), patterns=["stick_handling", "coordination"], muscles={"forearms": 0.8, "shoulders": 0.2}, equip=["hockey_stick"], stimulus="skill", phase="puck", uni=True, diff=4, vtype="dur", vol=20),
    E("Ведение с поднятой головой под воздействием партнёра", "Партнёр или тренер стоит в нескольких метрах напротив. Игрок ведёт мяч, удерживая взгляд на партнёре, пока партнёр время от времени перекатывает в его сторону другой предмет, который нужно заметить боковым зрением, не опуская глаз вниз.", stats=("puck_handling", "agility"), patterns=["stick_handling", "coordination"], muscles={"forearms": 0.6, "core": 0.2, "shoulders": 0.2}, equip=["hockey_stick"], stimulus="skill", phase="puck", diff=4, vtype="dur", vol=30),
    E("Ведение с резкой сменой ритма", "Вести мяч в медленном контролируемом темпе 5-6 секунд, затем без предупреждения резко перейти на максимально быструю частоту движений ещё на 5-6 секунд, снова вернуться к медленному темпу.", stats=("puck_handling", "agility"), patterns=["stick_handling", "coordination"], muscles={"forearms": 0.7, "shoulders": 0.2, "core": 0.1}, equip=["hockey_stick"], stimulus="skill", phase="puck", diff=4, vtype="dur", vol=30),
    E("Резкое поднятие мяча крюком на месте", "Мяч лежит на полу. Резким подсечным движением крюка приподнять мяч в воздух на небольшую высоту, поймать его на крюк и сразу повторить, не давая мячу упасть далеко.", stats=("puck_handling", "agility"), patterns=["stick_handling", "coordination"], muscles={"forearms": 0.7, "shoulders": 0.3}, equip=["hockey_stick"], stimulus="skill", phase="puck", diff=4, vtype="sr", vol=(3, 10, 15)),
    E("Ведение вдоль линии препятствий на скорости", "Расставить несколько предметов в ряд на небольшом расстоянии друг от друга. Максимально быстро вести мяч, обходя каждое препятствие змейкой, сохраняя контроль на высокой скорости движения ног.", stats=("puck_handling", "on_ice_skating"), patterns=["stick_handling", "locomotion", "coordination"], muscles={"forearms": 0.4, "calves": 0.3, "quads": 0.2, "core": 0.1}, equip=["hockey_stick"], stimulus="skill", phase="puck", diff=5, vtype="sr", vol=(1, 3, 4)),
    E("Ведение спиной вперёд", "Двигаясь спиной вперёд шагом или короткими приставными шагами, одновременно вести мяч клюшкой перед собой, сохраняя контроль при непривычном для контроля мяча направлении движения тела.", stats=("puck_handling", "agility"), patterns=["stick_handling", "locomotion"], muscles={"forearms": 0.5, "hamstrings": 0.2, "core": 0.2, "shoulders": 0.1}, equip=["hockey_stick"], stimulus="skill", phase="puck", diff=5, vtype="sr", vol=(1, 4, 5)),
    E("Комплексный финт с завершением", "Соединить в одной непрерывной последовательности: финт носком крюка, сразу перевод мяча под корпус на другую сторону, и завершающее резкое движение мяча вперёд (имитация паса или броска в стену).", stats=("puck_handling", "agility"), patterns=["stick_handling", "coordination", "rotation"], muscles={"forearms": 0.5, "core": 0.3, "shoulders": 0.2}, equip=["hockey_stick"], stimulus="skill", phase="puck", uni=True, diff=5, vtype="sr", vol=(3, 6, 8)),
    E("Ведение одной рукой под сопротивлением партнёра", "Держа клюшку одной рукой, удерживать контроль мяча, пока партнёр своей клюшкой пытается выбить или отобрать мяч, оказывая умеренное игровое сопротивление.", stats=("puck_handling", "strength", "agility"), patterns=["stick_handling", "coordination"], muscles={"forearms": 0.8, "core": 0.1, "shoulders": 0.1}, equip=["hockey_stick"], stimulus="skill", phase="puck", uni=True, diff=5, vtype="dur", vol=20),
]
