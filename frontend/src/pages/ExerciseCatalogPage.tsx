...
export function ExerciseCatalogPage() {
  ...
  // Two-screen drill-down, not tabs: null shows the skill list, picking a
  // skill pushes into that skill's own exercise list (with a back row to
  // return) -- replaces the old on_ice/off_ice category tabs and the
  // all-skills-stacked view, neither of which helped find one exercise in
  // a long catalog.
  ...
  const visibleExerciseIds = new Set(
    (exercises ?? []).filter((exercise) => hasExerciseDescription(exercise)).map((exercise) => exercise.id),
  )
  ...
  const selectedGroup = groups.find((group) => group.skill.id === selectedSkillId) ?? null
      ...
      <div className="relative z-[1] mx-auto flex max-w-2xl flex-col gap-6 px-4 py-8">
        ...
        {!isLoading && groups.length === 0 && (
          <EmptyState
            ...
            hint="Загляните позже — каталог ещё пополняется."
          />
        )}

        {!isLoading && groups.length > 0 && selectedGroup === null && (
          <div className="flex flex-col gap-2">
            {groups.map(({ skill, exercises: groupExercises }) => (
              <button
                ...
                className={`flex items-center justify-between gap-3 p-4 text-left transition-colors hover:border-white/20 ${CARD_CLASS}`}
              >
                <span className="text-sm font-medium text-[#F5F7FA]">{skill.name}</span>
                <span className="flex shrink-0 items-center gap-2 text-xs text-[#8A94A6]">
        ...
        {!isLoading && selectedGroup !== null && (
          <div className="flex flex-col gap-3">
            <button
              type="button"
              onClick={() => setSelectedSkillId(null)}
              className="flex items-center gap-1.5 self-start text-sm font-medium text-[#8A94A6] transition-colors hover:text-[#F5F7FA]"
            >
              <i className="ti ti-chevron-left" aria-hidden="true" />
              Все навыки
            </button>
            <h2 className="text-sm font-semibold uppercase tracking-wide text-[#8A94A6]">
              {selectedGroup.skill.name}
            </h2>
            <div className="flex flex-col gap-2">
