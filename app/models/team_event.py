...
class DrillTemplate(Base):
    """A drill a coach saved for reuse -- title, description, minutes and
    rink scheme, copied into a board as a brand-new TeamEventDrill (editing
    the copy never touches the template). Belongs to the coach, not a team,
    so it follows them from team to team. Capped per coach, see
    DrillTemplateService.MAX_TEMPLATES_PER_USER.
    """
    ...
    description: Mapped[str | None] = mapped_column(Text, nullable=True)
    duration_minutes: Mapped[int | None] = mapped_column(Integer, nullable=True)
    # Same shape as TeamEventDrill.diagram, validated by
    # schemas.team_event.DrillDiagram on write.
    diagram: Mapped[dict | None] = mapped_column(JSONB, nullable=True)
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), server_default=func.now(), nullable=False
    )
